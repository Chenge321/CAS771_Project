...
def groupClsData(dataCls, device): 
    ...
    batch_size = imagesCls.size()[0]
    cls1 = imagesCls.narrow(0, 0, batch_size //2)
    label1 = labelsCls.narrow(0, 0, batch_size //2)
    
    cls2 = imagesCls.narrow(0, batch_size //2, batch_size //2)
    label2 = labelsCls.narrow(0, batch_size //2, batch_size //2)
...
def LrWarmUp(trainLoaderCls, warmUpIter, lr, optimizer1, optimizer2, net_feat1, net_cls1, net_feat2, net_cls2, freeze_bn, dist, mask_feat_dim, dropout, device, history): 
    ...
    while nbIter < warmUpIter:      
        ...
        accuracy1 = utils.AverageMeter()
        accuracy2 = utils.AverageMeter()
        ...
        for batchIdx, dataCls in enumerate(trainLoaderCls):
            ...
            accuracy1.update(acc1Batch[0].item(), imagesCls1.size()[0])
            accuracy2.update(acc2Batch[0].item(), imagesCls2.size()[0])

            msg = 'Lr: {:.3f} | Loss1: {:.3f} | Loss2: {:.3f} | Acc1: {:.3f}% | Acc2: {:.3f}% '.format(\
                        lrUpdate, lossCls1.avg, lossCls2.avg, accuracy1.avg, accuracy2.avg)
    ...
    history['trainAccCls1'].append(accuracy1.avg)
    history['trainAccCls2'].append(accuracy2.avg)  
...
def Evaluation(EpochId, valLoaderCls, net_feat1, net_cls1, net_feat2, net_cls2, dist, mask_feat_dim, dropout, device, training_history): 
    ...
    training_history['valAccClsTotal'].append(acc)
    training_history['valK'].append(k)
    msg = '\nCo-teaching ... Epoch {:d}, Acc {:.3f} %, K {:d} | Best Acc {:.3f} %'.format(EpochId, acc * 100, k, training_history['bestAcc'][-1] * 100)
    print(msg)
